...
@main.route('/ding',methods=['GET'])
def infoindex():
	username = request.args.get("name")
	jessionid = request.args.get("jsessionid")
	strbyte = base64.b64decode(jessionid)
	permissionstr = strbyte.decode()
	permissionlist = permissionstr.split(',')
	session['permissons'] = permissionlist
	DataInfo = {"username": username, "permissionList": permissionlist}
	return render_template('main/authentication.html', DataInfo=DataInfo)
